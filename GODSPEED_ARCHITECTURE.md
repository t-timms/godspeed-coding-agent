...
For local Qwen3.5+ models (`qwen3.5`..`qwen3.9`, e.g. Qwen3.8-27B via llama.cpp), thinking is controlled through the chat template instead: Godspeed sends `chat_template_kwargs` with `enable_thinking` and `reasoning_effort`. `reasoning_effort` (`/effort none|low|medium|high`) maps to the template's `xhigh|medium|low` tiers (`none` turns thinking off; `high` is `xhigh`); with only `thinking_budget` set, the budget maps onto the same tiers (<=8192 medium, otherwise xhigh; a derived tier never picks `low`). With neither set, the template default applies, which for Qwen3.8 is thinking ON at `xhigh`. llama-server has no per-request token budget; use the server-wide `llamacpp.reasoning_budget` for a hard cap. Note: on Qwen3.8 an explicit `low` is honoured but does NOT reliably shorten reasoning (Prism ML measured it using about as many tokens as `xhigh`), so `medium` is the shortest effective tier; the `minimal` alias maps to `medium`.
